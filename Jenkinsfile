pipeline {
  agent {
    node {
      label 'master'
    }
  }
  parameters {
    string(name: 'distName', defaultValue: 'eventti' , description: '镜像名')
    string(name: 'distRepository', defaultValue: 'http://mvn.sky-cloud.net:8081/repository/sky-statics/eventti/', description: '静态文件发布位置')
  }
  stages {
    stage('Build') {
      steps {
        script {
           docker.withRegistry('http://hub.sky-cloud.net', 'docker-image-builder') {
            docker.image('hub.sky-cloud.net/cicd/gobuilder:v2.0').inside {
			  def GIT_COMMIT_HASH = sh(script: "git rev-parse --short HEAD", returnStdout: true).trim()
              def filter = sh(returnStdout: true, script: "expr substr $BRANCH_NAME 1 6").trim()
              if (filter == 'master') {
                  sh 'go version'
                  sh 'go env -w GOSUMDB=off'
                  sh 'GOPROXY=https://goproxy.cn go get ./...'
                  sh 'GOPROXY=direct go get -u github.com/sky-cloud-tec/proto'
                  sh 'GOPROXY=direct go get -u github.com/songtianyi/rrframework'
                  sh 'GOPROXY=https://goproxy.cn go build'
                  sh "echo ${GIT_COMMIT_HASH}"
                  def distNumber = sh(returnStdout: true, script: "echo $BRANCH_NAME | sed \"s/.*\\///g\"").trim()
                  def PACKAGE_NAME = "${params.distName}.${distNumber}_build-${GIT_COMMIT_HASH}"
                  sh "tar -zcvf ${params.distName}.${distNumber}_build-${GIT_COMMIT_HASH}.tar.gz ./eventti ; curl --fail -u admin:r00tme --upload-file ${params.distName}.${distNumber}_build-${GIT_COMMIT_HASH}.tar.gz ${params.distRepository}"
               }
            }
          }
        }
      }
    }
    stage('build-dockerimage') {
      steps {
        script {
		  def GIT_COMMIT_HASH = sh (script: "git rev-parse --short HEAD", returnStdout: true).trim()
          def filter = sh(returnStdout: true, script: "expr substr $BRANCH_NAME 1 6").trim()
          if (filter == 'master') {
		      sh 'echo ${GIT_COMMIT_HASH}'
		      sh "git rev-parse --short HEAD"
              def distNumber = sh(returnStdout: true, script: "echo $BRANCH_NAME | sed \"s/.*\\///g\"").trim()
              def PACKAGE_HOME = "${params.distRepository}${params.distName}.${distNumber}_build-${GIT_COMMIT_HASH}"
              sh "curl -u admin:r00tme -O ${params.distRepository}${params.distName}.${distNumber}_build-${GIT_COMMIT_HASH}.tar.gz ; tar -zxvf ${params.distName}.${distNumber}_build-${GIT_COMMIT_HASH}.tar.gz"
              sh "docker login -u docker-image-builder -p sky-cloud@SZ2018 hub.sky-cloud.net ; docker build -t hub.sky-cloud.net/sky/${params.distName}:${distNumber}_build-${BUILD_NUMBER} . ; docker push hub.sky-cloud.net/sky/${params.distName}:${distNumber}_build-${BUILD_NUMBER}"  
              sh "rm -rf eventti ; rm ${params.distName}.${distNumber}_build-${GIT_COMMIT_HASH}.tar.gz -fr"
              sh "sshpass -p r00tme ssh -o StrictHostKeyChecking=no root@192.168.1.146 \"docker pull hub.sky-cloud.net/sky/${params.distName}:${distNumber}_build-${BUILD_NUMBER} \""
              sh "sshpass -p r00tme ssh -o StrictHostKeyChecking=no root@192.168.1.146 \"kubectl set image deploy/eventti eventti=hub.sky-cloud.net/sky/${params.distName}:${distNumber}_build-${BUILD_NUMBER} -n sky\""
          }
        }
      }
    }
  }
}
